...
module.exports = {
  ...
  async executeSlash(interaction) {
    try {
      ...
      // Validar día y mes
      if (día < 1 || día > 31) {
        return interaction.reply({ content: 'Por favor, ingresa un día válido (1-31).', ephemeral: true });
      }
      if (mes < 1 || mes > 12) {
        return interaction.reply({ content: 'Por favor, ingresa un mes válido (1-12).', ephemeral: true });
      }
      ...
      if (birthdayExists) {
        return interaction.reply({ content: `Ya existe un cumpleaños registrado para **${displayName}**.`, ephemeral: true });
      }
      ...
      if (!birthdayChannelId) {
        return interaction.reply({ content: 'No se ha configurado un canal de cumpleaños. Un administrador debe usar el comando `/set-birthday-channel` para definirlo.', ephemeral: true });
      }

      // Guardar la fecha de cumpleaños en la base de datos
      await birthdaysDB.set(`${interaction.guild.id}.${id}`, { día, mes });

      await interaction.reply({ content: `El cumpleaños de **${displayName}** ha sido establecido para el **${día}/${mes}**.`, ephemeral: false });

    } catch (error) {
      console.error(error);
      await interaction.reply({ content: 'Ocurrió un error al intentar agregar el cumpleaños. Inténtalo nuevamente más tarde.', ephemeral: true });
    }
  },
};
