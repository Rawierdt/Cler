...
module.exports = {
  ...
  async executeSlash(interaction) {
    // Defer reply para evitar que la interacción expire si toma tiempo
    await interaction.deferReply({ ephemeral: true });

    // Verificar si el usuario tiene permisos de administrador
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      return interaction.editReply({ content: '<:win11erroicon:1287543137505378324> | No tienes permisos para usar este comando.' });
    }
    ...
    if (!canal || canal.type !== 0) { // 0 es el tipo de canal de texto
      return interaction.editReply({ content: '<:440warning:1287542257985126501> | Por favor selecciona un canal de texto válido.' });
    }
    ...
    if (!botPermissions.has(PermissionsBitField.Flags.ViewChannel) || !botPermissions.has(PermissionsBitField.Flags.SendMessages)) {
      return interaction.editReply({ content: '<:440warning:1287542257985126501> | No tengo permisos para ver o enviar mensajes en el canal seleccionado. Por favor, verifica mis permisos. <:toggleon:1287542502613712990>' });
    }

    // Guardar el canal en la base de datos
    await configDB.set(`birthdayChannel_${interaction.guild.id}`, canal.id);

    await interaction.editReply({ content: `<:win11checkicon:1287543060015612046> : Canal de cumpleaños establecido en <a:heartarrow_purple:1287542898266607669> ${canal}` });
  },
};
